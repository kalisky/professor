...
function chooseBoard() {
	...
	$('#page' + (numPages - 1)).append('<div class="boardLink ' + (solved ? "solved" : "unsolved") + '" onclick="showBoard('+data.boards[i]+');"><div class="boardLinkTxt">'+(i+1)+'</div>' + fill + '</div>');
	numBoardsInPage ++;
    }
}
function showBoard(id) {
    closeMsg();
    if (id == 1) {
	    groups=[['חתול','כלב','אוגר','ארנב'],['צפי','הדר','לימור','לילך'],['ציקי','ארז','ניר','חנן'],['עיר החטאים','יום ראשון','בית ענק','סנשרון מטבח']];
	    connectionsRaw = ['חיות!חיות מחמד!','נשים','קארפול!קרפול','מסעדות!חדרי אוכל'];
    } else if (id == 2) {
        groups=[['טרונדהיים','מלבורן','בון','איסטנבול'],['אויר','סדום','דונה','מנצ׳סטר'],['לונדון','מינכן','רומא','סידני'],['ברצלונה','פריז','קהיר','מנהטן']];
	    connectionsRaw = ['בירות לשעבר!ערי בירה','שיער!המחזמר שיער','אולימפיאדה!ערים אולימפיות','וודי אלן'];
    } else if (id == 3) {
		...
		connectionsRaw = ['אלאניס מוריסט!אלאניס','ג׳וליה לואי דרייפוס!ג׳וליה לואי!ג׳וליה','מספר!מספרים','ערים תאומות!באר שבע!עיר תאומה באר שבע'];
    }
    connections = [];
